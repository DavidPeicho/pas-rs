...
macro_rules! tests {
    ($slice: ident, $name: ident, $borrow: ident) => { paste::expr! {
        ...
        fn [<slice_len_$name>]() {
            ...
            let slice: $slice<[f32; 2]> = $slice::new(vertices.$borrow(), std::mem::size_of::<[f32; 3]>()).strided(2);
        ...
        fn [<strided_$name>]() {
            ...
            let positions: $slice<[u32; 3]> = $slice::new(data.$borrow(), 0).strided(stride);
            assert!(positions.iter().eq([[1, 2, 3], [7, 8, 9]].iter()));

            let normals: $slice<[u32; 3]> = $slice::new(data.$borrow(), 3 * std::mem::size_of::<u32>()).strided(stride);
